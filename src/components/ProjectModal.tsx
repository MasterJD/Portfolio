...
export default function ProjectModal({ project, onClose }: Props) {
  ...
  const projectCopy = project
    ? projectList[String(project.id)]
    : null

  const titleText = projectCopy
    ? (lang === 'en' ? projectCopy.title.english : projectCopy.title.spanish)
    : project?.icon || 'Project'
  const descriptionText = projectCopy
    ? (lang === 'en' ? projectCopy.description.english : projectCopy.description.spanish)
    : ''
  const purposeText = projectCopy
    ? (lang === 'en' ? projectCopy.purpose.english : projectCopy.purpose.spanish)
    : ''
  const statusText = projectCopy
    ? (lang === 'en' ? projectCopy.status.english : projectCopy.status.spanish)
    : ''
      ...
      <div className="modal-content relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white dark:bg-black border-4 border-black dark:border-white scale-100 opacity-100">
        ...
        <div className="p-6">
              ...
              <div className="flex items-center gap-6">
                {hasMultiple && (
                  <button
                    ...
                    aria-label={tKey('dialogs.prevImage')}
                  >
                ...
                {hasMultiple && (
                  <button
                    onClick={nextImage}
                    className="shrink-0 w-12 h-12 flex items-center justify-center border-2 border-black dark:border-white text-black dark:text-white hover:bg-primary hover:border-primary hover:text-white transition-all"
                    aria-label={tKey('dialogs.nextImage')}
                  >
          ...
          {purposeText && (
            <div className="mb-6">
              <h4 className="text-black dark:text-white text-sm font-bold uppercase tracking-widest mb-2">
                {tKey('dialogs.purpose')}
              </h4>
              <p className="text-black/70 dark:text-white/70 text-sm leading-relaxed">{purposeText}</p>
            </div>
          )}

          <div className="mb-6">
            <h4 className="text-black dark:text-white text-sm font-bold uppercase tracking-widest mb-3">
              {tKey('dialogs.technologies')}
            </h4>
          ...
          {statusText && (
            <div className="mb-6">
              ...
              <span className={`inline-block px-3 py-1 text-xs font-bold uppercase tracking-wider border-2 ${
                statusText === 'Completed' || statusText === 'Completado'
          ...
          <div className="flex flex-wrap gap-4">
            {project.demoEnabled && (
              ...
              >
                ...
                <span>{tKey('dialogs.liveDemo')}</span>
              </a>
            )}
